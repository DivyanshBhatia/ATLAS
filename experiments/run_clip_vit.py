"""
Cross-Backbone Validation: CLIP ViT-B/16
...
Usage:
    python run_clip_vit.py
    python run_clip_vit.py --exp task_structure
    python run_clip_vit.py --exp comparison
    python run_clip_vit.py --exp all
"""
import sys
sys.path.insert(0, '.')
...
# CLIP ViT-B/16 config
CLIP_MODEL = "vit_base_patch16_clip_224"
SUP_IMG_SIZE = 224
SUP_EMBED_DIM = 768
SUP_NUM_LAYERS = 12
SUP_NUM_HEADS = 12
...
def run_task_structure(model, device):
    ...
    print("\n" + "=" * 70)
    print("TASK STRUCTURE — CLIP ViT-B/16")
    print("=" * 70)
...
def run_comparison(model, config, device):
    ...
    print("\n" + "=" * 70)
    print("PEFT COMPARISON — CLIP ViT-B/16")
    print("=" * 70)
...
def main():
    ...
    model = timm.create_model(CLIP_MODEL, pretrained=True, img_size=SUP_IMG_SIZE).to(device)
    ...
    if 'task_structure' in all_results and 'comparison' in all_results:
        print(f"\n{'='*70}")
        print(f"CROSS-BACKBONE: DINOv2 vs CLIP ViT-B")
        print(f"{'='*70}")
    ...
    # Save
    save_path = os.path.join(config.output_dir, 'clip_vit_results.json')
