"""
Cross-Backbone Validation: BEiTv2 ViT-B/16
...
Usage:
    python run_beitv2_vit.py
    python run_beitv2_vit.py --exp task_structure
    python run_beitv2_vit.py --exp comparison
    python run_beitv2_vit.py --exp all
"""
import sys
sys.path.insert(0, '.')
...
# BEiTv2 ViT-B/16 config
BEITV2_MODEL = "beitv2_base_patch16_224"
SUP_IMG_SIZE = 224
SUP_EMBED_DIM = 768
SUP_NUM_LAYERS = 12
SUP_NUM_HEADS = 12
...
def run_task_structure(model, device):
    ...
    print("\n" + "=" * 70)
    print("TASK STRUCTURE — BEiTv2 ViT-B/16")
    print("=" * 70)
...
def run_comparison(model, config, device):
    ...
    print("\n" + "=" * 70)
    print("PEFT COMPARISON — BEiTv2 ViT-B/16")
    print("=" * 70)
...
def main():
    ...
    config.model_name = BEITV2_MODEL
    ...
    print(f"Loading {BEITV2_MODEL}...")
    model = timm.create_model(BEITV2_MODEL, pretrained=True, img_size=SUP_IMG_SIZE).to(device)
    ...
    all_results = {'model': BEITV2_MODEL, 'sigma_p_sq': sigma_p_sq}
    ...
    if 'task_structure' in all_results and 'comparison' in all_results:
        print(f"\n{'='*70}")
        print(f"CROSS-BACKBONE: DINOv2 vs BEiTv2 ViT-B")
        print(f"{'='*70}")
    ...
    # Save
    save_path = os.path.join(config.output_dir, 'beitv2_vit_results.json')
