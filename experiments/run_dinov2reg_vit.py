"""
Cross-Backbone Validation: DINOv2-reg ViT-B/14
...
Usage:
    python run_dinov2reg_vit.py
    python run_dinov2reg_vit.py --exp task_structure
    python run_dinov2reg_vit.py --exp comparison
    python run_dinov2reg_vit.py --exp all
"""
import sys
sys.path.insert(0, '.')
...
# DINOv2-reg ViT-B/14 config
DINOREG_MODEL = "vit_base_patch14_reg4_dinov2.lvd142m"
SUP_IMG_SIZE = 224
SUP_EMBED_DIM = 768
SUP_NUM_LAYERS = 12
SUP_NUM_HEADS = 12
...
def run_task_structure(model, device):
    ...
    print("\n" + "=" * 70)
    print("TASK STRUCTURE — DINOv2-reg ViT-B/14")
    print("=" * 70)
...
def run_comparison(model, config, device):
    ...
    print("\n" + "=" * 70)
    print("PEFT COMPARISON — DINOv2-reg ViT-B/14")
    print("=" * 70)
...
def main():
    ...
    config.model_name = DINOREG_MODEL
    ...
    print(f"Loading {DINOREG_MODEL}...")
    model = timm.create_model(DINOREG_MODEL, pretrained=True, img_size=SUP_IMG_SIZE).to(device)
    ...
    all_results = {'model': DINOREG_MODEL, 'sigma_p_sq': sigma_p_sq}
    ...
    if 'task_structure' in all_results and 'comparison' in all_results:
        print(f"\n{'='*70}")
        print(f"CROSS-BACKBONE: DINOv2 vs DINOv2-reg ViT-B")
        print(f"{'='*70}")
    ...
    # Save
    save_path = os.path.join(config.output_dir, 'dinov2reg_vit_results.json')
