"""
Cross-Backbone Validation: MAE ViT-B/16
...
Usage:
    python run_mae_vit.py
    python run_mae_vit.py --exp task_structure
    python run_mae_vit.py --exp comparison
    python run_mae_vit.py --exp all
"""
import sys
sys.path.insert(0, '.')
...
def run_task_structure(model, device):
    ...
    print("\n" + "=" * 70)
    print("TASK STRUCTURE — MAE ViT-B/16")
    print("=" * 70)
...
def run_comparison(model, config, device):
    ...
    print("\n" + "=" * 70)
    print("PEFT COMPARISON — MAE ViT-B/16")
    print("=" * 70)
...
def main():
    ...
    print(f"Loading {MAE_MODEL}...")
    model = timm.create_model(MAE_MODEL, pretrained=True, img_size=SUP_IMG_SIZE).to(device)
    ...
    all_results = {'model': MAE_MODEL, 'sigma_p_sq': sigma_p_sq}
    ...
    if 'task_structure' in all_results and 'comparison' in all_results:
        print(f"\n{'='*70}")
        print(f"CROSS-BACKBONE: DINOv2 vs MAE ViT-B")
        print(f"{'='*70}")
    ...
    # Save
    save_path = os.path.join(config.output_dir, 'mae_vit_results.json')
