"""
Cross-Backbone Validation: DeiT-III ViT-B/16
...
Usage:
    python run_deit3_vit.py
    python run_deit3_vit.py --exp task_structure
    python run_deit3_vit.py --exp comparison
    python run_deit3_vit.py --exp all
"""
import sys
sys.path.insert(0, '.')
...
# DeiT-III ViT-B/16 config
DEIT3_MODEL = "deit3_base_patch16_224"
SUP_IMG_SIZE = 224
SUP_EMBED_DIM = 768
SUP_NUM_LAYERS = 12
SUP_NUM_HEADS = 12
...
def run_task_structure(model, device):
    ...
    print("\n" + "=" * 70)
    print("TASK STRUCTURE — DeiT-III ViT-B/16")
    print("=" * 70)
...
def run_comparison(model, config, device):
    ...
    print("\n" + "=" * 70)
    print("PEFT COMPARISON — DeiT-III ViT-B/16")
    print("=" * 70)
...
def main():
    ...
    print(f"Loading {DEIT3_MODEL}...")
    model = timm.create_model(DEIT3_MODEL, pretrained=True, img_size=SUP_IMG_SIZE).to(device)
    ...
    all_results = {'model': DEIT3_MODEL, 'sigma_p_sq': sigma_p_sq}
    ...
    if 'task_structure' in all_results and 'comparison' in all_results:
        print(f"\n{'='*70}")
        print(f"CROSS-BACKBONE: DINOv2 vs DeiT-III ViT-B")
        print(f"{'='*70}")
    ...
    # Save
    save_path = os.path.join(config.output_dir, 'deit3_vit_results.json')
