...
# ============================================================
# EXPERIMENT 3: iBOT Backbone Control
# ============================================================
def _load_ibot_state_dict(checkpoint_path):
    """Load an official iBOT checkpoint and return backbone-only weights."""
    checkpoint = torch.load(checkpoint_path, map_location="cpu")

    if isinstance(checkpoint, dict) and "state_dict" in checkpoint:
        state_dict = checkpoint["state_dict"]
    elif isinstance(checkpoint, dict) and "teacher" in checkpoint and isinstance(checkpoint["teacher"], dict):
    ...
    for key, value in state_dict.items():
        ...
        # The teacher checkpoint contains the iBOT prediction head. Priority 3
        # tests only the pretrained ViT-B/16 backbone, so exclude head weights.
...
def _build_ibot_model(device, ibot_root, checkpoint_state):
    """Construct official iBOT ViT-B/16 and load backbone weights."""
    ...
    if "vit_base" not in ibot_models.__dict__:
        raise RuntimeError(
            f"Official iBOT package at {ibot_models.__file__} does not expose vit_base"
        )
    ...
    msg = model.load_state_dict(checkpoint_state, strict=False)

    missing = list(getattr(msg, "missing_keys", []))
    unexpected = list(getattr(msg, "unexpected_keys", []))
    if missing or unexpected:
        raise RuntimeError(
            "iBOT backbone checkpoint mismatch: "
...
def _compute_ibot_sigma(model):
    """Compute sigma_P using the same definition as revision2_ssl_control."""
    d = model.embed_dim
    per_layer = []
    for block in model.blocks:
        W_qkv = block.attn.qkv.weight.float()
...
def _compute_ibot_gradient_metric(model, data_loader, device, config, num_prompts=5):
    """Compute linearized VPT gradient magnitude, matching revision2 SSL control."""
...
def run_ibot(device, config, checkpoint_path=None):
    """Run the actual iBOT Priority-3 control using the revision2 SSL protocol."""
    print(f"\n{'=' * 60}")
    print("iBOT BACKBONE CONTROL EXPERIMENT")
    print(f"{'=' * 60}")
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not import official iBOT models from {ibot_root}: "
            f"{type(e).__name__}: {e}"
        ) from e

    print(f"  Architecture factory: {ibot_models.__file__}")
    ...
    if checkpoint_path is None:
        raise FileNotFoundError(
            "iBOT checkpoint not found. Pass --ibot_checkpoint /path/to/checkpoint_teacher.pth"
        )

    print(f"  Checkpoint: {checkpoint_path}")
    checkpoint_state = _load_ibot_state_dict(checkpoint_path)

    # Verify the official backbone before starting training.
    sanity_model = _build_ibot_model(device, ibot_root, checkpoint_state).eval()
    with torch.no_grad():
        x = torch.randn(1, 3, 224, 224, device=device)
        y = sanity_model(x)
    print(f"  Forward pass: OK, output shape={tuple(y.shape)}")
    del sanity_model, x, y
    torch.cuda.empty_cache()

    # Match revision2_ssl_control exactly for comparability.
    img_size = 224
    test_tasks = ['cifar100', 'svhn', 'gtsrb', 'eurosat', 'dtd']
    save_path = 'results/revision3_ibot_control.json'
    os.makedirs('results', exist_ok=True)
    ...
    reference_model = _build_ibot_model(device, ibot_root, checkpoint_state)
    config.embed_dim = reference_model.embed_dim
    config.num_layers = len(reference_model.blocks)
    config.num_heads = reference_model.blocks[0].attn.num_heads
    config.head_dim = reference_model.embed_dim // reference_model.blocks[0].attn.num_heads
    sigma_p = _compute_ibot_sigma(reference_model)
    vpt_lr = 1e-3 if sigma_p < 0.5 else 1e-2
    del reference_model
    torch.cuda.empty_cache()

    all_results.setdefault('iBOT', {
        'sigma_p': float(sigma_p),
        'source': f'official:{checkpoint_path}',
        'vpt_lr': float(vpt_lr),
        'is_dino': False,
        'is_ssl': True,
        'protocol': 'revision2_ssl_control',
        'tasks': {},
    })

    print(f"  sigma_P = {sigma_p:.4f}, VPT LR = {vpt_lr:.0e}")
    print("  SSL: True, DINO: False")
    print("  Protocol: LoRA-r8 + VPT-p5 + gradient metric, seed=42")

    for task in test_tasks:
        if task not in TASKS:
            continue
        if task in all_results['iBOT']['tasks']:
        ...
        # LoRA-r8: identical to revision2_ssl_control.
        model = _build_ibot_model(device, ibot_root, checkpoint_state)
        model.head = nn.Linear(config.embed_dim, num_classes).to(device)
        model = apply_lora(model, 8, config).to(device)
        config.lr = 1e-3
        lora_acc = train_and_evaluate(model, train_loader, val_loader, config, device)
        del model
        torch.cuda.empty_cache()

        # VPT-p5: identical fixed LR rule to revision2_ssl_control.
        model = _build_ibot_model(device, ibot_root, checkpoint_state)
        model.head = nn.Linear(config.embed_dim, num_classes).to(device)
        try:
            model = apply_vpt(model, 5, config).to(device)
            config.lr = vpt_lr
            vpt_acc = train_and_evaluate(model, train_loader, val_loader, config, device)
        except Exception as e:
            raise RuntimeError(
                f"VPT failed for iBOT task={task}: {type(e).__name__}: {e}"
            ) from e
        del model
        torch.cuda.empty_cache()

        # Linearized VPT gradient metric: same first 5 batches as revision2.
        model_tmp = _build_ibot_model(device, ibot_root, checkpoint_state)
        model_tmp.head = nn.Linear(config.embed_dim, num_classes).to(device)
        grad_mag = _compute_ibot_gradient_metric(
            model_tmp,
            val_loader,
            device,
            config,
        ...
        winner = (
            'L'
            if lora_acc > vpt_acc + 0.02
            else 'V' if vpt_acc > lora_acc + 0.02
            else 'T'
        )

        all_results['iBOT']['tasks'][task] = {
            'lora': float(lora_acc),
            'vpt': float(vpt_acc),
            'winner': winner,
        ...
        print(
            f"    {task:<12s}: LoRA={lora_acc:.3f} VPT={vpt_acc:.3f} "
    ...
    # Summary identical in spirit to revision2_ssl_control.
    tasks_data = all_results['iBOT']['tasks']
    wins = {'L': 0, 'T': 0, 'V': 0}
    grad_vals = []
    for result in tasks_data.values():
        w = result['winner']
        if w.startswith('L'):
            wins['L'] += 1
        elif w.startswith('V'):
            ...
            wins['T'] += 1
        if result['grad_mag'] > 0:
            grad_vals.append(result['grad_mag'])

    all_results['iBOT']['wins'] = wins
    all_results['iBOT']['mean_grad'] = float(np.mean(grad_vals)) if grad_vals else -1.0

    with open(save_path, 'w') as f:
        json.dump(all_results, f, indent=2, default=str)

    print(f"\n{'=' * 60}")
    print("iBOT CONTROL SUMMARY")
    print(f"{'=' * 60}")
    print(
        f"  iBOT: sigma_P={sigma_p:.4f}, "
        f"L/T/V={wins['L']}/{wins['T']}/{wins['V']}, "
        f"mean grad={all_results['iBOT']['mean_grad']:.2f}"
    )
